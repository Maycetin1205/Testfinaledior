...
export function sendBwLink(command: string): boolean {
  ...
  return typeof g.basisHTML_SND_MSG === 'function'
    && hostCall(() => g.basisHTML_SND_MSG('HTMLEVENT', { art: 'BWLINK', params: row }))
}

export function sendStartTool(nr: string, params: readonly string[]): boolean {
  if (nr.trim() === '') return false
  const g = seWindow()
  const message: Record<string, unknown> = { NR: nr }
  ...
  return typeof g.sendBWLinkIntern === 'function'
    && hostCall(() => g.sendBWLinkIntern(startToolLink(nr, params)))
}
